...
we *could* write these lists by ourselves.
...
such as [YAML Lint](https://www.yamllint.com/),
...
![](fig/team_members_list.png){alt="Rendered team members list in the 'about.md' page" .image-with-shadow width="800px" }
...
In [the *Working with Filters* episode]({{ relative\_root\_path }}/filters/) we
...
The mechanism Jekyll provides to do this is called a *collection*:
...
the `collections` configuration is a [*nested mapping*][yaml-lists]:
...
![](fig/blog_posts_list.png){alt='Rendered blog post list in the index page' .image-with-shadow width="800px" }
