...
[create formatted webpage content with Markdown]({{ relative\_root\_path }}/markdown/),
[configure a GitHub repository to use the Jekyll static site generator]({{ relative\_root\_path }}/github-pages/),
and [how to use Jekyll to build our Markdown pages and various other files]({{ relative\_root\_path }}/starting-jekyll/)
...
![](fig/directory-structure-rendered-website-with-annotations.png){alt='Directory structure of a jekyll website containing this lesson' .image-with-shadow height="1200px" }
...
![](fig/gh-fork-button.png){alt='the GitHub interface fork button' height="50px"}

If you belong to many GitHub organizations, you might have to
choose which account to copy/fork the repo to.

![](fig/github-forking-progress.png){alt='GitHub copying the fork' height="200px"}
...
![](fig/github-branch-ghpages.png){alt='GitHub branching dropdown menu with gh-pages typed in box' height="300px"}
...
2. Also remove the "Share this" block to the bottom of the post where the newsletter block used to be (above "Author Box").
...
2. To move the "Share this" box, lines 45-65 should be cut (copied then deleted) and then pasted between the Article and the Author Box section.
...
2. Make the `About` page content include the site description you updated
  and any other text you'd like to add.
3. Update the header menu to include a link for the `About` page.
...
2. In the body of the `_pages/about.md` description variable is added using liquid syntax, `{% raw %} {{ site.description }} {% endraw %}`, and add any other text that might be helpful for an about page.
3. The header is updated in the `_includes/menu-header.html` (this may take some searching to track down).  The same html from the other items in the header can be copied and modified.
